...
# compute predictions on the test cohort for model 55
pred55<-exp(as.matrix(test_var)%*%as.matrix(allcoef[,which(model_col==TRUE)[[1]]]))

# compute statistics: Concordance index for model 55
library(survcomp)
concordance.index(x=pred55,  surv.time=test_surv$OS, surv.event=test_surv$STATUS, method="noether")$c.index


#compute Mean,Max,Min of coeff for the 55 model
